...
import { getClientsByPhase } from "@/server/queries/airtable-clients";
import { formatNumber } from "@/lib/format";

export async function ClientPhasesSection() {
  const t = await getTranslations("phases");
  const tErr = await getTranslations("errors");
  const locale = await getLocale();

  let data;
  try {
    data = await getClientsByPhase();
  } catch (err) {
    return (
